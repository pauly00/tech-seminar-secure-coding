...
// admin' -- 입력 시 실제 실행되는 쿼리:
// SELECT * FROM user_tb WHERE user_id = 'admin' --' AND user_password = '...'
// 비밀번호 조건이 주석 처리되어 우회됨

// 안전 코드 (UserService.java)
String sql = "SELECT * FROM user_tb WHERE user_id = ? AND user_password = ?";
jdbcTemplate.queryForList(sql, userId, password);
...
// 템플릿에서 th:utext 로 출력하면 <script> 태그가 그대로 실행됨

// 안전 코드 (XssController.java)
secureComments.add(comment);  // 동일하게 저장하지만
// 템플릿에서 th:text 로 출력 시 Thymeleaf가 자동으로 HTML 인코딩 처리
// <script> → &lt;script&gt; 로 변환되어 브라우저가 태그로 인식하지 않음
```
...
<!-- 안전: Thymeleaf가 출력 전 HTML 엔티티로 인코딩 -->
<span th:text="${comment}"></span>
```

**수정 포인트**: `th:utext` → `th:text` 로 변경
...
Path dest = dir.resolve(originalFilename);  // 원본 파일명 그대로 사용
Files.copy(file.getInputStream(), dest, StandardCopyOption.REPLACE_EXISTING);

// 안전 코드 (FileService.java)
...
**수정 포인트**: `if (!ALLOWED_EXTENSIONS.contains(extension)) return "차단";`
...
String requestedPath = safeFilesDir + "/" + filename;  // 검증 없음
Files.readString(Path.of(requestedPath));

// 안전 코드 (PathTraversalController.java)
...
**수정 포인트**: `if (!requestedPath.startsWith(basePath)) throw ...;`
...
|--------|---------------------|----------|-----------------|
| SQL Injection | A03 Injection | CVE-2017-5638 | `jdbcTemplate.queryForList(sql, userId, pw)` |
| XSS | A03 Injection | CVE-2019-11358 | `th:text` (Thymeleaf HTML 자동 인코딩) |
